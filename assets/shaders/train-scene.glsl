...
// Foreground clouds retain the original fill-below-contour construction.
#define layer(dh, v)  if (uv.y < h + midlevel - (dh) ) return vec4(v, 1.);
// Only the most distant cloud is a finite band floating in open sky.
#define distantLayer(dh, v) if (uv.y < h + midlevel - (dh) && uv.y > h + midlevel - (dh) - 0.055) return vec4(v, 1.);
// Disabled pending a separate, explicitly requested high-cloud treatment.
#define upperLayer(dh, v)
...
vec4 background(vec2 uv, float t){
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    distantLayer(0.14, vec3(0.48, 0.19, 0.20));
    distantLayer(0.1, vec3(0.68, 0.28, 0.19));
    distantLayer(0.07, vec3(0.88, 0.38, 0.24));
    distantLayer(0., vec3(0.95, 0.45, 0.30));
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    layer(0.04, vec3(0.68, 0.78, 0.89));
    layer(0., vec3(0.94, 0.97, 1.0));
    ...
    // c9 (removed: warm band at the bridge horizon)
    midlevel = -10.0;
    disp = 2.0;
    dist = 25.0;
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    layer(0.1, vec3(0.95, 0.66, 0.48));
    layer(0.05, vec3(0.98, 0.76, 0.64));
    layer(0., vec3(0.95, 0.80, 0.77));
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    upperLayer(0.2, vec3(0.92, 0.85, 0.82));
    upperLayer(0.15, vec3(0.95, 0.80, 0.77));
    upperLayer(0.1, vec3(0.98, 0.76, 0.64));
    upperLayer(0., vec3(1.0, 0.94, 0.91));
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    upperLayer(0.04, vec3(0.92, 0.85, 0.82));
    upperLayer(0., vec3(0.95, 0.80, 0.77));
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    upperLayer(0.1, vec3(0.95, 0.80, 0.77));
    upperLayer(0., vec3(1.0, 0.94, 0.91));
    ...
    h = (fbm(uv2, 8) - 0.5)*disp;
    upperLayer(0.1, vec3(0.92, 0.85, 0.82));
    upperLayer(0.05, vec3(0.95, 0.80, 0.77));
    upperLayer(0., vec3(1.0, 0.94, 0.91));
